<script lang="ts">
    ...
    function handleObservationsClick(event: Event, patientId: string) {
        ...
        console.log(`Navigating to Observations for patient ID: ${patientId}`); // Debugging
        navigate(`/observations/${patientId}`);
    }

    function handleAppointmentsClick(event: Event, patientId: string) {
        event.stopPropagation(); // Prevent row click
        console.log(`Navigating to Appointments for patient ID: ${patientId}`); // Debugging
        navigate(`/appointments/${patientId}`);
    }

    function handleQuestionnairesClick(event: Event, patientId: string) {
        event.stopPropagation(); // Prevent row click
        console.log(`Navigating to Questionnaires for patient ID: ${patientId}`); // Debugging
        navigate(`/questionnaires/${patientId}`);
    }

    function getNhsNumber(identifiers: any[]): string {
        if (!identifiers) return 'N/A'; // Check if identifiers is undefined
        const nhsIdentifier = identifiers.find(identifier => 
            identifier.system === "https://fhir.nhs.uk/Id/nhs-number"
        );
        return nhsIdentifier ? nhsIdentifier.value : 'N/A';
    }
            ...
            <tbody>
                {#each patients as patient}
                    <tr on:click={() => handlePatientClick(patient.id)}>
                        <td>{patient.id}</td>
                        <td>{getNhsNumber(patient.identifier)} </td>
                        ...
                        <td>
                            <button class="observations-button" on:click={(e) => handleQuestionnairesClick(e, patient.id)}>Questionnaires</button>
                        </td>
                    </tr>
                {/each}
            </tbody>
